...
export default function AdminSMSPage() {
  ...
  const handleSend = async () => {
    ...
    if (!token) return toast.error("❌ Ви не авторизовані");
    ...
    try {
      ...
      if (data.success) {
        toast.success(`✅ ${data.message || "Розсилку надіслано!"}`);
        setMessage("");
      } else {
        toast.error(data.message || "⚠️ Помилка розсилки");
      }
    } catch (err) {
    ...
    <section className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-black text-white flex flex-col items-center justify-center p-6">
      ...
      >
        <div className="flex items-center gap-2 mb-5">
          ...
          <h1 className="text-2xl font-bold text-green-400">SMS Розсилка</h1>
        </div>
        ...
        >
          {sending ? (
            <>
              <Loader2 size={18} className="animate-spin" /> Надсилання...
            </>
          ) : (
            <>
              <Send size={18} /> Надіслати SMS
            </>
          )}
        </motion.button>
      </motion.div>
